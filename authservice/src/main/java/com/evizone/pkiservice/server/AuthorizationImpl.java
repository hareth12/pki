...
import com.evizone.pkiservice.EnrollInput;
import com.evizone.pkiservice.OpenXPKIService;
...
public class AuthorizationImpl implements Authorization {
	...
	private com.evizone.pkiservice.OpenXPKI openxpki = new OpenXPKIService().getOpenXPKIPort();
	
	@Override
	public String ejbcaRegister(
	...
	@Override
	public String openxpkiRegister(
			@WebParam(name = "user") AuthorizationRegisterInput user) {
		if (isInputValid(user)) {
			Connection conn = sqlUtil.getJNDIConnection(datasourceContext);
			try {
				if (conn != null) {
					if (!sqlUtil.userExists(conn, user.getCn())) {
						...
						if (res == 2) {
							/*
							 * Enrollment request sent to OpenXPKI
							 */
							sendEnrollmentRequest(user.getCn());
							/*
							 * Confirmation email with one-time-password
							 */
							sendConfirmationEmail(user.getCn(), user.getEmail(), password);
	...
	private void sendEnrollmentRequest(String cn) {
		try {
			EnrollInput input = new EnrollInput();
			input.setCn(cn);
			String msg = openxpki.enroll(input);
			System.out.println("Enrollment request response: " + msg);
		} catch (Exception e) {
			e.printStackTrace();
